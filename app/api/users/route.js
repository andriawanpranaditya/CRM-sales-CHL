...
export async function GET() {
  ...
  const sql = db();
  const rows = await sql`SELECT id, username, name, role, active, created_at FROM users ORDER BY role, name`;
...
export async function POST(req) {
  ...
  await sql`INSERT INTO users (username, name, role, password_hash) VALUES (${b.username}, ${b.name}, ${role}, ${hash})`;
